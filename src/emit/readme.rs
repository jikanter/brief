...
pub fn emit_readme(brief: &Brief) -> String {
    ...
    if has_constraints {
        ...
        for constraint in &brief.constraints.hard {
            out.push_str(&format!(
                "- {} **(REQUIRED)**\n",
                with_scope(constraint, constraint)
            ));
        }
        for constraint in &brief.constraints.soft {
            out.push_str(&format!(
                "- {} *(preferred)*\n",
                with_scope(constraint, constraint)
            ));
        }
        for constraint in &brief.constraints.ask_first {
            out.push_str(&format!(
                "- {} **(ASK FIRST)**\n",
                with_scope(constraint, constraint)
            ));
    ...
    if !brief.assumptions.is_empty() {
        ...
        for assumption in &brief.assumptions {
            let marker = if assumption.validated { "[x]" } else { "[ ]" };
            out.push_str(&format!("- {marker} {}\n", assumption.text));
        }
        out.push('\n');
    }

    if let Some(deliverable) = &brief.deliverable {
