...
**Status (2026-06-21): DECIDED — Direction 1 shipped (P8).** Scope is now a format-level model field: `Constraint { text, scope: Vec<Glob> }`, empty scope = project-wide. Authoring syntax is the flat bracket prefix (`` [`src/ui/**`] text ``; globs backtick-wrapped so Markdown does not mangle `**`). `Constraint::is_directory_prefix` exposes the directory-prefix vs. arbitrary-glob distinction. The `cursor` emitter fans scoped constraints out into per-glob `brief-<slug>.mdc` files with native `globs:` frontmatter (proving the split-by-scope mechanism); every other emitter renders scope non-destructively (inline `When working in …:` prose, or a structured `scope="…"` attribute for `xml`). JSON carries the `scope` array. `brief validate` warns on scopes that match no files. Native fan-out for `copilot`/`windsurf`/`.claude/rules` is deferred (reuses the cursor mechanism). See [brief-format.md](brief-format.md) §8.1.
